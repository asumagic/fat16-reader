...
const std::map<uint8_t, std::string> MBRPartition::partition_types
{
	{0x1, "FAT12"},
	{0x4, "FAT16"},
	{0x5, "Étendue"},
...
const std::string MBRPartition::unknown_type = "Inconnu";

const std::string& MBRPartition::type_name() const // @TODO make this less bad
{
	try
	{
...
MBR::MBR(std::ifstream& dump)
{
	...
	dump.read(reinterpret_cast<char*>(partitions.data()), sizeof(MBRPartition) * 4);

	mbrlog << trivial << "Listage et vérification des partitions" << std::endl;
	for (const MBRPartition& part : partitions)
	{
		if (part.type)
